...
## Added Llama3 model integration for RGB evaluation
## ============================================================================
import sys
import os

# Add LLM folder to path for Llama3 model
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', 'LLM'))


class Llama3:
    """
    Llama-3.1-8B-Instruct model wrapper for RGB evaluation.
    Requires: LLM/llama_model.py and HuggingFace token in LLM/hg file.
    """
    def __init__(self, plm='meta-llama/Llama-3.1-8B-Instruct') -> None:
        from llama_model import load_model, generate_text as llama_generate, compute_logprob_and_states
        self.plm = plm
        self.llama_generate = llama_generate
        self.compute_logprob_and_states = compute_logprob_and_states
        # Load model on initialization
        load_model(plm)
        
    def get_logprob_and_states(self, text, target_answer, system="", return_all_layers=False, return_token_logprobs=False):
        if len(system) > 0:
            full_prompt = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{text}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
        else:
            full_prompt = f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{text}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
            
        return self.compute_logprob_and_states(
            prompt=full_prompt,
    ...
    def generate(self, text, temperature=0.7, system="", top_p=0.9, max_new_tokens=512):
        """
        Generate text using Llama-3.1-8B-Instruct.
        
        Args:
            text: Input prompt (with documents and query)
            temperature: Sampling temperature
            system: System prompt (prepended to text)
        ...
        """
        # Combine system prompt and user text (Llama-3.1 format)
        if len(system) > 0:
            full_prompt = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n{system}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n{text}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
        else:
            full_prompt = f"<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\n{text}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
        
        response = self.llama_generate(
            prompt=full_prompt,
