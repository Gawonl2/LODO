...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        ## MODIFIED BY ME - Removed choices restriction to allow custom datasets (e.g., en_mid_confident, en_mid_conversational)
        help='evaluation dataset (e.g., en, zh, en_int, zh_int, en_fact, zh_fact, en_small, en_mid, en_mid_confident, etc.)'
    )
    parser.add_argument(
    ...
    instances = []
    ## MODIFIED BY ME - Added encoding='utf-8' for Windows compatibility
    ...
    ## MODIFIED BY ME - Added Llama3 model support
    elif 'llama3' in modelname.lower() or 'llama-3' in modelname.lower():
        model = Llama3(plm = args.plm)
    ...
    json.dump(scores,open(f'{resultpath}/prediction_{args.dataset}_{modelname}_temp{temperature}_noise{noise_rate}_passage{passage_num}_correct{args.correct_rate}_result.json','w', encoding='utf-8'),ensure_ascii=False,indent=4)
